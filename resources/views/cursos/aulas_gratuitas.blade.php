...
<head>
  ...
  <style>
    ...
    /* ESTILOS DO MODAL */
    #loginModal .modal-content, #certModal .modal-content, #unlockModal .modal-content { background-color: var(--bg-dark); }
    #loginModal .form-control, #certModal .form-control, #unlockModal .form-control { background-color: #333; border: 1px solid #555; color: white; }
    #loginModal .form-control:focus, #certModal .form-control:focus, #unlockModal .form-control:focus { background-color: #333; border-color: var(--brand-green); box-shadow: 0 0 0 0.25rem rgba(40, 167, 69, 0.5); color: white; }
    .plyr { border-radius: 8px;min-width: 100%; }

    /* NOVO: ESTILOS PARA ITENS BLOQUEADOS */
    .playlist-item.locked { opacity: 0.6; cursor: pointer; }
    .playlist-item.locked:hover { background-color: rgba(231, 76, 60, 0.1); }
    .playlist-item.locked .item-title { color: var(--text-muted); }
    .playlist-item .lock-icon { font-size: 1.2rem; color: var(--locked-color); }
    
    /* NOVO: ESTILOS PARA OS CABEÇALHOS DOS MÓDULOS */
    .module-header-item {
        background-color: #252525; padding: 0.75rem 1rem; margin-top: 1.5rem; margin-bottom: 0.5rem;
        border-radius: 6px; font-weight: bold; color: var(--text-light);
        display: flex; align-items: center; gap: 0.75rem;
    }

    /* Fixar a barra do certificado no rodapé (apenas mobile) */
    ...
    .certificate-section{
        ...
        margin: 0 !important;
        border-radius: 0;
        z-index: 1040; /* acima do header (1030) */
...
<body>
                    ...
                    <div id="playlist-list">
                        ...
                        @if($modulosCompletos)
                            @foreach($modulosCompletos as $modulo)
                                ...
                                @if(isset($modulo['topics']))
                                    @foreach($modulo['topics'] as $topico)
                                        <a href="#" class="playlist-item locked" data-bs-toggle="modal" data-bs-target="#unlockModal">
                                            <i class="bi bi-lock-fill lock-icon"></i>
                                            <div class="item-info">
                                                <p class="item-title">{{ $topico }}</p>
                                            </div>
                                        </a>
                                    @endforeach
                                @endif
    ...
    <!-- NOVO MODAL: Desbloquear Curso Completo -->
    <div class="modal fade" id="unlockModal" tabindex="-1" aria-labelledby="unlockModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content text-light border-secondary">
                <div class="modal-header border-bottom-secondary">
                    <h5 class="modal-title fw-bold" id="unlockModalLabel"><i class="bi bi-gem me-2 text-warning"></i> Desbloqueie o Curso Completo</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body text-center">
                    <img src="{{asset('/storage/'.$curso->capa_quadrada)}}" alt="{{$curso->titulo}}" class="img-fluid rounded-circle mb-3" style="width: 120px; height: 120px; object-fit: cover;">
                    <p class="lead">Gostou do que viu? Inscreva-se para ter acesso a <strong>todas as aulas, bônus e ao certificado.</strong></p>
                    <p>Continue sua jornada e torne-se um profissional qualificado!</p>
                </div>
                <div class="modal-footer border-top-secondary justify-content-center">
                    <a href="{{$curso->link_checkout_completo}}&offDiscount=50OF" class="btn-certificate w-100">
                        <i class="bi bi-unlock-fill me-2"></i>QUERO DESBLOQUEAR TUDO AGORA
                    </a>
                </div>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $('#form-login-whatsapp').on('submit', function (e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    console.error('Erro ao salvar lead:', xhr.responseJSON);
                }
            });
        });
        ...
        // Espaço no fim da página para não ficar nada escondido atrás da barra fixa (só mobile)
        if (window.matchMedia('(max-width: 576px)').matches) {
        const cert = document.querySelector('.certificate-section');
        if (cert) {
            const applyPadding = () => {
            document.body.style.paddingBottom = (cert.offsetHeight + 12) + 'px';
            };
            applyPadding();
            window.addEventListener('resize', applyPadding);
            document.addEventListener('shown.bs.modal', applyPadding);
            document.addEventListener('hidden.bs.modal', applyPadding);
        }
        }
