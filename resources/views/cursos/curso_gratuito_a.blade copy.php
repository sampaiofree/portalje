...
<body style="box-sizing: border-box;">
    ...
    <section class="container-fluid bg-portal py-5 text-white">
        <div class="row">
            <div class="col-12">
        ...
        <div class="row">
            <!-- Área do vídeo -->
            @php
                $primeira_capa = $curso->video_apresentacao ?? $curso->video_dentro_do_curso;
                $primeira_capa = $primeira_capa 
                    ? "https://img.youtube.com/vi/{$primeira_capa}/hqdefault.jpg" 
                    : asset('/storage/' . $curso->capa_horizontal);

                $videotopo = $curso->aulas_demonstrativas[0]['aula_id_youtube'] ;//$curso->video_apresentacao ?? $curso->video_dentro_do_curso;
            ...
            <div class="col-12 col-lg-8">
                <div id="modal_video_aula_demonstrativa" class="video-facade" data-video-id="{{$videotopo}}" onclick="loadVideo(this)">
                    <img loading="lazy" src="{{$primeira_capa}}" alt="{{$curso->titulo}}" class=" img-fluid img-lazy">
            ...
            <div class="mt-3 col-12 col-lg-4">
                <h6 class="fw-bolder">Clique no título da aula:</h6>
                <ul class="list-group list-group-flush">
                    @if(isset($curso->aulas_demonstrativas) AND !empty($curso->aulas_demonstrativas))
                    @foreach($curso->aulas_demonstrativas->take($n_aulas) as $aulas_demonstrativas)
                ...
                <div class="mt-3 text-center">
                    <p class="fw-bold mb-2" style="line-height: 1.3">Após assistir todas as aulas <br>aperte no botão abaixo para emitir o seu certificado.</p>
                    <!--<a href="https://wa.me/556291945608?text=Quero o meu certificado do curso de {{$curso->titulo}}" class="btn-cta text-center text-uppercase fw-bolder">Emitir meu Certificado Agora</a>-->
                    <a data-bs-toggle="modal" data-bs-target="#certModal" class="btn-cta text-center text-uppercase fw-bolder">Emitir meu Certificado Agora</a>
...
<div class="modal fade" id="certModal" tabindex="-1" aria-labelledby="certModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content border-0 shadow-lg">
        ...
        <div class="modal-body">
          <p>Emitir um certificado válido, personalizado e digital exige uma estrutura técnica — com registro, segurança, emissão automática e suporte.</p>
          <p>Oferecemos o curso 100% gratuito, mas pedimos um valor simbólico para emissão do certificado:</p>
          <ul>
            <li>✅ Garante autenticidade com seu nome e data</li>
            <li>📄 Carga horária reconhecida</li>
            <li>📥 Arquivo em PDF disponível imediatamente</li>
            <li>🧾 Suporte e validação digital</li>
          </ul>
          <p><strong>💡 Resultado:</strong> você recebe um certificado profissional, pronto para currículo, horas complementares, processos seletivos e mais.</p>
        </div>
        <div class="modal-footer justify-content-between">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
          <a href="{{$curso->link_checkout_completo}}" class="btn btn-success btn-cta">Quero meu Certificado Agora</a>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="modal-dialog modal-fullscreen">
        ...
        <form id="form-login-whatsapp" style="max-width: 400px; width: 100%;">
            ...
            <!-- Imagem no topo -->
            <div class="mb-3">
            <img src="{{asset('/storage/'.$curso->capa_quadrada)}}" alt="{{$curso->titulo}}" class="img-fluid rounded" style="width: 150px; height: 150px; object-fit: cover;">
            </div>

            <!-- Título do curso -->
            <h4 class="mb-4 fw-bolder">{{$curso->titulo}}</h4>
            <p>Preecha os dados abaixo para começar o seu curso.</p>

             <!-- Campo de nome -->
            <div class="mb-3 text-start">
                <label for="nome" class="form-label">Digite o nome que irá no certificado</label>
                <input type="text" class="form-control" id="nome" name="nome" placeholder="Digite seu Nome" required>
            </div>

            <!-- Campo de WhatsApp -->
            <div class="mb-3 text-start">
                <label for="whatsapp" class="form-label">Digite o seu número de WhatsApp</label>
                <div id="erro-whatsapp" class="text-danger mt-1" style="display: none; font-size: 14px;"> Por favor, digite um WhatsApp válido com.</div>
                <input type="text" class="form-control" id="whatsapp" name="whatsapp" placeholder="(99) 99999-9999" required>
                <p class="text-muted" style="font-size: x-small">Ao fazer o login você concorda em receber avisos pelo WhatsApp.</p>
            </div>
            ...
            <!-- Botão -->
            <button type="submit" class="btn btn-success dw-bolder w-100">ACESSAR O CURSO</button>
  ...
  white-space: nowrap;
    ...
    <script>
        ...
        $('#form-login-whatsapp').on('submit', function (e) {
            ...
            // Captura origem dinâmica da URL
        ...
        function changeVideo(videoUrl) {
            ...
            if (!videoId) {
                console.warn("ID do vídeo não encontrado:", videoUrl);
                return;
            }
