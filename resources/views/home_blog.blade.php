...
<html lang="pt-br">
  <head>
    ...
    @if (request()->getHost() === 'portalje.org' OR request()->getHost() === 'jovemempreendedor.org' OR request()->getHost() === 'jempreendedor.com' OR request()->getHost() === '3f7.org')
...
@media (min-width: 769px) {
  ...
  <body style="box-sizing: border-box;">
    ...
    <div class="container-fluid">
        <p class="text-center" style="font-size:xx-small">Anúncio</p>
        <!-- vertical_responsivo -->
        <ins class="adsbygoogle"
            style="display:block"
            data-ad-client="ca-pub-9796869151117705"
            data-ad-slot="3841172467"
            data-ad-format="auto"
    ...
    <section id="beneficios" class="bg-portal3 color1 lazy-load" style="padding-bottom: 50px; padding-top: 50px;">
        <div class="container-fluid">
            <div class="row">
                ...
                <!-- Título da sessão -->
                    <h3 id="beneficios_h3" class="fw-bold">Quem pode participar?</h3>
                    @if($info['cidade'])
                    <p class="lead text-center">O Programa Nacional oferece bolsas parciais para cursos profissionalizantes EAD, destinadas exclusivamente a moradores de {{$info['cidade']}} e região.</p>
                    @else
                    <p class="lead text-center">O Programa Nacional oferece bolsas parciais para cursos profissionalizantes EAD. Confira abaixo os requisitos.</p>
                    @endif 
            ...
            <div class="row mt-3">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <p class=" mb-0 "><i class="bi bi-calendar-check"></i> Idade mínima de 14 anos (sem limite máximo)</p>
                    </li>
                    <li class="list-group-item bg-transparent my-1">
                    <p class="mb-0 "><i class="bi bi-globe "></i> Ter um celular com acesso à internet para acompanhar as aulas</p>
                    </li>
                    <li class="list-group-item bg-transparent my-1">
                    <p class="mb-0 "><i class="bi bi-book "></i> Não há requisitos de escolaridade, mas é fundamental ter a capacidade de leitura.</p>
                    </li>
                </ul>
                @if($info['cidade'])
                <p class="mt-3 lead text-center">Apenas os candidatos residentes em <strong>{{$info['cidade']}} ou região,</strong> que atendem aos critérios podem ser selecionados.</p>
                @endif
                </div>
            </div>
    ...
    <!-- Full screen modal -->
    <div id="quiz" class="modal" tabindex="-1" style="z-index: 9999999">
            ...
            <div class="modal-content">
                <div class="modal-body modal-dialog-centered">
                    ...
                    <div class="p-4 border rounded bg-light">
                        ...
                        <div id="quizProgress" class="progress-bar progress-bar-striped" role="progressbar" style="width: 33%;" aria-valuenow="33" aria-valuemin="0" aria-valuemax="100">1/3</div>
                        </div>

                        <!-- PERGUNTA 1 -->
                        <div class="quiz-step" id="step-1">
                            <h4>Qual seu nível de escolaridade?</h4>
                            <button class="btn d-block btn-outline-primary btn-block mb-2 next-btn" data-next="step-2">Fundamental</button>
                            <button class="btn d-block btn-outline-primary btn-block mb-2 next-btn" data-next="step-2">Média</button>
                            <button class="btn d-block btn-outline-primary btn-block mb-2 next-btn" data-next="step-2">Superior</button>
                        </div>

                        <!-- PERGUNTA 2 -->
                        <div class="quiz-step d-none" id="step-2">
                            <h4>Qual o melhor horário para você estudar?</h4>
                            <button class="btn d-block btn-outline-success btn-block mb-2 next-btn" data-next="step-3">De manhã</button>
                            <button class="btn d-block btn-outline-success btn-block mb-2 next-btn" data-next="step-3">À tarde</button>
                            <button class="btn d-block btn-outline-success btn-block mb-2 next-btn" data-next="step-3">À noite</button>
                        </div>

                        <!-- PERGUNTA 3 -->
                        <div class="quiz-step d-none" id="step-3">
                            <h4>O que você espera conquistar?</h4>
                            <button class="btn d-block btn-outline-warning btn-block mb-2 "  data-bs-dismiss="modal" aria-label="Close">Um emprego melhor</button>
                            <button class="btn d-block btn-outline-warning btn-block mb-2 "  data-bs-dismiss="modal" aria-label="Close">Mais conhecimento</button>
                            <button class="btn d-block btn-outline-warning btn-block mb-2 "  data-bs-dismiss="modal" aria-label="Close">Um certificado no currículo</button>
                        </div>
                    </div>
                </div>
...
<div id="pre_modal_ads" class="modal show" tabindex="-1" style="display: inline-block">
...
<div id="modal_ads" class="modal show" tabindex="-1" style="display: inline-block">
  <div class="modal-dialog modal-fullscreen-sm-down">
    <div id="modal_content_ads" class="modal-content " style="height: 100%; display: flex; flex-direction: column;">
      ...
      <div class="mt-5 text-center">
        <p id="aguardeTexto" style="font-size: 14px; color: #888; margin-top: 10px;">Aguarde para continuar (15s)</p>
        <button id="btn_fechar_ads" type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal" style="display: none;">Fechar</button>
      </div>
         ...
         * ##################################*/ 
         document.addEventListener('DOMContentLoaded', function() {
            ...
            jquery.onload = function () {
                // Só depois de jQuery carregado, carregamos o Bootstrap
                ...
                bootstrapJS.onload = function () {
                    // Agora sim, jQuery e Bootstrap estão prontos — pode abrir o modal
                    $('#quiz').modal('show');
                };
        ...
        * ##################################*/ 
        window.onload = function() {
            ...
            $('.next-btn').click(function() {
                    const current = $(this).closest('.quiz-step');
                    ...
                    const index = steps.indexOf(nextId);
            ...
            const btn = document.getElementById('btn_fechar_ads');
            const texto = document.getElementById('aguardeTexto');

            // 3. Temporizador do botão fechar
            let s = 15;
            const intervalo = setInterval(() => {
                s--;
                texto.innerText = `Aguarde para continuar (${s}s)`;
                if (s <= 0) {
            ...
            document.getElementById('btn_fechar_ads').addEventListener('click', () => {
                /*const modalEl = document.getElementById('ads');
                const modal = bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
                modal.hide();*/
                document.getElementById('modal_ads').remove();
                document.getElementById('pre_modal_ads').remove();
