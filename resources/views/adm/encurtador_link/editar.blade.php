...
<div class="row justify-content-center">
    <div class="col-12">
        <h4 class="mb-4">Editar Link</h4>
        
        <!-- Formulário para encurtar a URL -->
        <form id="form_editar_link_encurtado" action="{{route('encurtar_link_editar_alterar', ['id'=>$link->id])}}" method="POST">
            
            @csrf
            <!-- Campo de URL longa -->
            <div class="mb-3">
                <label for="url_longa" class="form-label">URL de destino</label>
                <input type="url" class="form-control" id="url_longa" name="url_longa" required value="{{$link->url_longa}}">
            </div>

            <div class="mb-3">
                <label class="form-label">Slug - Personalizar link encurtado</label>
                <div class="input-group flex-nowrap">
                    <span class="input-group-text" id="basic-addon1">
                        @if(Auth::user()->dominio_externo)
                        {{Auth::user()->dominio_externo}}

                        @else
                        {{Auth::user()->dominio}}
                        @endif
                        /e/</span>
                    <input type="text" class="form-control" aria-label="Username" name="slug" aria-describedby="basic-addon1" value="{{$link->slug}}">
                </div>
            </div>
...
<script>
    ...
    // Intercepta o evento de submissão do formulário
        $('#form_editar_link_encurtado').on('submit', function(event) {
