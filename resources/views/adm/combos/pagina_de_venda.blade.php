...
@media (min-width: 769px) {
  ...
  <body style="box-sizing: border-box;">
   ...
   <!-- Cabeçalho -->
    ...
    <section class="hero" style="box-sizing: border-box; justify-content: center !important; align-items: center !important; display: flex !important;">
        ...
        <h2 id="headline" class="fw-bold mb-0 mt-4 color4 mx-auto" style="color: #ffffff; text-shadow: 1px 1px 10px rgb(0, 0, 0);font-size: 1.3rem;"><strong>Veja tudo o que você vai aprender no Combo {{$combo->titulo}}</h2>
            <p class="mt-1 mb-4" style="color: white; text-shadow: 1px 1px 10px rgb(0, 0, 0);line-height: 1.2;font-size: 1rem;" class="mt-0 fs-4">{{$combo->headline}}</p>
        <a href="#sessao_cursos" class="btn-cta text-uppercase" style="text-align: center !important;font-size: 1rem;">Quero conhecer</a> 
    ...
    <section id="sessao_cursos" class="lazy-load py-5 color1">
        <div class="container-fluid">
            <h3 id="beneficios_h3" class="fw-bold text-center">São {{count($combo->cursos)}} cursos completos com certificado válido e linguagem simples. </h3>
            <p class="lead text-center">Veja os detalhes de cada um abaixo:</p>
            ...
            <div id="lista_cursos" class="row mx-auto mt-3" style="max-width: 1100px;">
                 ...
                 @foreach($combo->cursos as $curso)
                    ...
                    <div class="lazy-load col-12 col-sm-6 col-lg-4 mb-2 mt-1 py-1 pb-3">
                        <div class="row row_curso_individual align-items-center d-flex">
                            ...
                            <div class="col-6 position-relative">
                                    ...
                                    <div class="img-gradient-overlay">
                                        <div class="img-title text-white">
                                            ...
                                            <!--<p class="my-0 d-flex justify-content-center align-items-center color4" style="font-size: x-small">
                                                ...
                                                Salário pode chegar a R${{ $curso->salario_maximo+1134 }}
                                            </p>-->
                                        </div>
                                ...
                                <span class="position-absolute d-flex  align-items-center translate-middle badge bg-danger" style="left: 80% !important;top: 6% !important;font-size: x-small;">
                            ...
                            <div class="col-6">
                                  <!-- Accordion -->
                                <div class="accordion" id="accordion-{{ $curso->id }}">
    
                                    <p class="mb-2" style="font-size: 0.8rem;line-height: 1.3;font-weight: 400">{{ str_replace('"', '', $curso['headline']) }} </p>

                                    <!-- O que você vai aprender -->
                                    <div class="accordion-item">
                                    <h2 class="accordion-header" id="headingOne-{{ $curso->id }}">
                                        <button class="accordion-button collapsed p-2" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne-{{ $curso->id }}" aria-expanded="false" aria-controls="collapseOne-{{ $curso->id }}" style="font-size: 0.85rem;">
                                        O que você vai aprender?
                                        </button>
                                    </h2>
                                    <div id="collapseOne-{{ $curso->id }}" class="accordion-collapse collapse" aria-labelledby="headingOne-{{ $curso->id }}" data-bs-parent="#accordion-{{ $curso->id }}">
                                        <div class="accordion-body p-2" style="font-size: 0.8rem; line-height: 1.3;">
                                        @if(isset($curso->conteudo) && count($curso->conteudo))
                                            <ul class="list-unstyled mb-0">
                                            @foreach($curso->conteudo as $conteudo)
                                                @if(isset($conteudo) && count($conteudo))
                                                <li class="mb-1">• {{ $conteudo['title'] }}</li>
                                                @endif
                                            @endforeach
                                            </ul>
                                        @else
                                    ...
                                    <div class="accordion-item">
                                        <h2 class="accordion-header" id="headingTwo-{{ $curso->id }}">
                                            <button class="accordion-button collapsed p-2" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo-{{ $curso->id }}" aria-expanded="false" aria-controls="collapseTwo-{{ $curso->id }}" style="font-size: 0.85rem;">
                                            Onde você pode trabalhar?
                                            </button>
                                        </h2>
                                        <div id="collapseTwo-{{ $curso->id }}" class="accordion-collapse collapse" aria-labelledby="headingTwo-{{ $curso->id }}" data-bs-parent="#accordion-{{ $curso->id }}">
                                            <div class="accordion-body p-2" style="font-size: 0.8rem; line-height: 1.3;">
                                              @if(!empty($curso->areas_de_atuacao))
                                                <ul class="list-unstyled mb-0">
                                                  @foreach(explode('/', $curso->areas_de_atuacao) as $area)
                                                    <li>• {{ ucfirst(trim($area)) }}</li>
                                                  @endforeach
                                                </ul>
                                              @else
                                                <p>Informações sobre mercado de trabalho não disponíveis.</p>
                                              @endif
                                            </div>
                                          </div>
            ...
            <div class="row">
                <div class="col-sm-10 col-md-8 col-lg-6 mx-auto">
                    <div class="row flex-nowrap overflow-auto centralizar">
                        <div class=" col-12 p-3 bg-portal border rounded rounded-3 mx-auto">
                            <h2 class="text-center" style="font-weight: bolder;color: #ffffba;font-size: 1.5rem;">FAÇA SUA INSCRIÇÃO</h2>
                            <div class="row">
                                <div class="text-center">
                                        <h3 class=" fw-bolder" style="font-size: 1rem;">O que você vai receber ao se inscrever?</h3>
                                        <p class="lead">Tudo que você precisa para conquistar seu emprego dos sonhos em {{count($combo->cursos)}} cursos!</p>
                                        <p>Ao se inscrever no combo {{ ucfirst(strtolower($combo->titulo)) }}, você garante acesso a:</p>
                                </div>
                            </div>
                            <div class="row">
                                <div class="">
                                    ...
                                    <li class="d-flex align-items-center my-1 px-3">
                                        ...
                                        <span class="fw-normal text-white" style="font-size: small;">
                                            Acesso completo aos {{ count($combo->cursos) }} cursos:
                                            @foreach($combo->cursos as $curso)
                                                {{ $curso['titulo'] }}{{ $loop->last ? '.' : ', ' }}
                                            @endforeach
                                        </span>
                                    ...
                                    <li class="d-flex align-items-center my-1 px-3">
                                        <i class="bi bi-check-circle-fill text-white fs-4 me-3"></i>
                                        <span class="fw-normal text-white" style="font-size: small;">{{ count($combo->cursos) }} certificados. Cada curso com Certificado de Conclusão reconhecido, pronto para turbinar o seu currículo.</span>
                                    </li>
                                    <li class="d-flex align-items-center my-1 px-3">
                                        <i class="bi bi-check-circle-fill text-white fs-4 me-3"></i>
                                    ...
                                    <li class="d-flex align-items-center my-1 px-3">
                                        <i class="bi bi-check-circle-fill text-white fs-4 me-3"></i>
                                        <span class="fw-normal text-white" style="font-size: small;">Curso Preparatório para o Mercado de Trabalho em vídeo aulas.</span>
                                    </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="row mt-5">
                                <div class="text-center text-white">
                                        ...
                                        <!--<p class="lead text-center">Além de tudo isso, você terá ao curso e seus materiais, podendo voltar sempre que precisar rever algum conteúdo ou atualizar suas habilidades!</p>-->
                                        <!--<p class="old-price my-0" style="font-size: small">de R${{$combo->preco}},00 por</p>-->
                                        
                                        <p class="mt-3 fs-7 mb-0">Tudo isso de <span class="fs-6 fw-bold text-danger" style="text-decoration: line-through;">${{$combo->preco}},00</span> por apenas</p>
                                        <p class="mb-0 fw-bolder display-5 color4" style="margin-top: -10px">
                                            {{$combo->preco_parcelado}}
                                        </p>
                                        <p class=" fs-6" style="margin-top: -10px">
                                            ou {{$combo->preco}} à vista
                                        </p>
                
                                        <div class="mt-4 text-center">
                                            <p class="fw-bold mb-2" style="line-height: 1.3">Inscreva-se agora mesmo e comece agora sua jornada rumo ao sucesso!</p><a href="{{$combo->link_checkout}}@if( $combo->origem != 'whatsapp')&sck=plano_completo @endif" class="btn-cta text-center text-uppercase btn-inscricao">Quero Me Inscrever Agora</a>
