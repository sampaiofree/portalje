...
class MinhaJornada
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if (Auth::check()) {
            ...
            $user = User::find($id); //BUSCAR DADOS DO USUÁRIO
            
            $summary = $user->approvedCompletedPurchaseEventsSummary(); //TOTAL EM VENDAS E TOTAL DE VENDAS

            $ref = Codigo_ref::where('user_id', $user->id)->first(); //VER SE AFILIADO CADASTROU PELO MENOS UM CÓDIGO REF

            $codigo_ref = new Codigo_ref();
            $codigos_ref = $codigo_ref->where('user_id', $user->id)->pluck('codigo_ref'); //PEGAR CÓDIGOS REF DO AFILIADO
            
            $ativos = $codigo_ref->where('user_id', $user->id)->where('mostrar_curso', '<>', '')->where('mostrar_curso', '<>', '0')->whereNotNull('mostrar_curso')->get(); //SE TEM CURSOS ATIVOS OU NÃO
            if($ativos->isEmpty()){$cursos_ativos = false;}else{$cursos_ativos = true;}; //DEFINIR VARIÁVEL DE CURSOS_ATIVOS
            
            $leads = PurchaseEvent::whereIn('affiliate_code', $codigos_ref)->first(); //SABER SE TEM LEADS

            $totalLeads = PurchaseEvent::whereIn('affiliate_code', $codigos_ref)
            ->distinct('buyer_checkout_phone') // Garante que apenas telefones únicos sejam considerados
            ->count();


            $vencidos = PurchaseEvent::whereIn('affiliate_code', $codigos_ref)->where('event', 'PURCHASE_EXPIRED')->get(['purchase_original_offer_price_value']); //SABER SE TEM LEADS
            $vencidos['n'] = $vencidos->count();
            $vencidos['soma'] = $vencidos->sum('purchase_original_offer_price_value');

            $aguardadndo = PurchaseEvent::whereIn('affiliate_code', $codigos_ref)->where('event', 'PURCHASE_BILLET_PRINTED')->get(['purchase_original_offer_price_value']); //SABER SE TEM LEADS
            $aguardadndo['n'] = $aguardadndo->count();
            $aguardadndo['soma'] = $aguardadndo->sum('purchase_original_offer_price_value'); //

            $canceladas = PurchaseEvent::whereIn('affiliate_code', $codigos_ref)->where('event', 'PURCHASE_CANCELED')->get(['purchase_original_offer_price_value']); //SABER SE TEM LEADS
            $canceladas['n'] = $canceladas->count();
            $canceladas['soma'] = $canceladas->sum('purchase_original_offer_price_value'); //PURCHASE_CANCELED

            $quantidade_vendas = $summary['total_count'];
            ...
            $minha_jornada = [
                [
                    "concluido" => $user->dominio ?? $user->dominio_externo,
                ...
                [
                    ...
                    "aulas" => [
                        ...
                        [
                            "titulo" => "Como funciona o plano básico?",
                            "link" => "lX5mxC4qUQE",
                            "texto" => "",
                        ],
                        [
            ...
            if($quantidade_vendas){
                $conversaoVendas = round(($quantidade_vendas / $totalLeads)*100, 2) ?? null;                
            }else{
